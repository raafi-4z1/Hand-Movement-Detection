...
def train(hyp, opt, device, tb_writer=None, wandb=None, logger=None):
...
def mainTrain(data="data.yaml", 
    ...
    opt = argparse.Namespace(adam=False,
                             batch_size=2,  # disesuaikan sebisa komputer/leptop
                             bucket='',
                             cache_images=False,
                             cfg=cfg,
                             data=data,
    ...
    # Hyperparameters
    hyp = {'lr0': lr,  # initial learning rate (SGD=1E-2, Adam=1E-3)
    ...
    if 'box' not in hyp:
        warn('Compatibility: Hyperparameter missing "box" which was renamed from "giou" in %s' %
                ('https://github.com/ultralytics/yolov5/pull/1120'))
    ...
    if not opt.evolve:
        ...
        train(hyp, opt, device, tb_writer, wandb, logger=logger)
    ...
    # Evolve hyperparameters (optional)
        ...
        for _ in range(300):  # generations to evolve
            ...
            # Train mutation
            results = train(hyp.copy(), opt, device, wandb=wandb, logger=logger)
